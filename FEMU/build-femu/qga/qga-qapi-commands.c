...
static void qmp_marshal_output_GuestLoadAverage(GuestLoadAverage *ret_in,
                                QObject **ret_out, Error **errp)
{
    Visitor *v;

    v = qobject_output_visitor_new_qmp(ret_out);
    if (visit_type_GuestLoadAverage(v, "unused", &ret_in, errp)) {
        visit_complete(v, ret_out);
    }
    visit_free(v);
    v = qapi_dealloc_visitor_new();
    visit_type_GuestLoadAverage(v, "unused", &ret_in, NULL);
    visit_free(v);
}
...
void qmp_marshal_guest_get_load(QDict *args, QObject **ret, Error **errp)
{
    ...
    qmp_marshal_output_GuestLoadAverage(retval, ret, errp);
