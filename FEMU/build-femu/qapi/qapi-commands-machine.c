...
static void qmp_marshal_output_QemuTargetInfo(QemuTargetInfo *ret_in,
                                QObject **ret_out, Error **errp)
{
    Visitor *v;

    v = qobject_output_visitor_new_qmp(ret_out);
    if (visit_type_QemuTargetInfo(v, "unused", &ret_in, errp)) {
        visit_complete(v, ret_out);
    }
    visit_free(v);
    v = qapi_dealloc_visitor_new();
    visit_type_QemuTargetInfo(v, "unused", &ret_in, NULL);
    visit_free(v);
}
...
void qmp_marshal_query_target(QDict *args, QObject **ret, Error **errp)
{
    ...
    qmp_marshal_output_QemuTargetInfo(retval, ret, errp);
